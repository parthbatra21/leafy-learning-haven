...
const ActivityZone: React.FC<ActivityProps> = ({ subject, onBack }) => {
  ...
  // Update the welcome card
  if (!subject) {
    return (
      <Card className="h-full flex items-center justify-center relative overflow-hidden bg-gradient-to-br from-blue-100 to-blue-200 border-none shadow-sm rounded-xl">
        <div className="absolute inset-0">
          {generateStars()}
        </div>
        <div className="text-center p-8 z-10">
          <h2 className="text-2xl font-bold mb-4 text-blue-800">Welcome to the Laboratory of Knowledge!</h2>
          <p className="text-lg mb-6 text-blue-700">Select an experiment station to begin your scientific journey!</p>
          <div className="flex justify-center space-x-4">
            <div className="planet-icon bg-blue-300/70 p-3 rounded-full animate-pulse">
              <FileSpreadsheet className="text-blue-800" size={36} />
            </div>
            <div className="planet-icon bg-purple-300/70 p-3 rounded-full animate-pulse delay-100">
              <BookOpen className="text-purple-800" size={36} />
            </div>
            <div className="planet-icon bg-orange-300/70 p-3 rounded-full animate-pulse delay-200">
              <User className="text-orange-800" size={36} />
            </div>
            <div className="planet-icon bg-pink-300/70 p-3 rounded-full animate-pulse delay-300">
              <Heart className="text-pink-800" size={36} />
            </div>
            <div className="planet-icon bg-indigo-300/70 p-3 rounded-full animate-pulse delay-400">
              <Brain className="text-indigo-800" size={36} />
            </div>
          </div>
        </div>
      </Card>
  ...
  <Button 
    ...
    size="sm" 
    onClick={onBack}
    className="mb-4 bg-blue-200/50 backdrop-blur-sm hover:bg-blue-300/50 text-blue-800"
  >
    ← Back to Laboratory Map
  </Button>
  ...
  return (
    <Card className="h-full flex flex-col relative overflow-hidden bg-gradient-to-br from-blue-100 to-blue-200 border-none shadow-sm rounded-xl">
      <div className="absolute inset-0">
        {generateStars()}
      </div>
      <div className="p-6 flex-1 z-10">
        ...
        <div className="flex items-center justify-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-blue-800">{currentActivity.title}</h1>
        </div>
